...
class BidirectionalTemporalMambaBlock(nn.Module):
    """Bidirectional Mamba over the time axis for each spatial location."""

    def __init__(self, channels, num_block=1):
        super(BidirectionalTemporalMambaBlock, self).__init__()
    ...
    def forward(self, x):
        if x.dim() != 5:
            raise RuntimeError(
                f"BidirectionalTemporalMambaBlock expects [B,T,C,H,W], got {tuple(x.shape)}."
            )
        ...
        b, t, _, _, _ = forward_features.shape
        gate = torch.sigmoid(
            self.gate(
                rearrange(
...
class SpatialTemporalMambaBlock(nn.Module):
    """Factorized temporal and spatial Mamba processing for event features."""

    def __init__(self, channels, num_block=1):
        super(SpatialTemporalMambaBlock, self).__init__()
        block_count = max(num_block, 1)
        self.temporal_mamba = BidirectionalTemporalMambaBlock(
            channels, num_block=block_count
        )
        self.spatial_blocks = nn.Sequential(
            *[EAMambaBlock(dim=channels) for _ in range(block_count)]
        )
        self.fusion_gate = nn.Conv2d(channels * 2, channels, kernel_size=1, stride=1)

    def forward(self, x):
        if x.dim() != 5:
            raise RuntimeError(
                f"SpatialTemporalMambaBlock expects [B,T,C,H,W], got {tuple(x.shape)}."
            )

        b, t, _, _, _ = x.shape
        temporal_features = self.temporal_mamba(x)
        spatial_features = rearrange(x, "b t c h w -> (b t) c h w")
        spatial_features = self.spatial_blocks(spatial_features)
        spatial_features = rearrange(spatial_features, "(b t) c h w -> b t c h w", b=b, t=t)

        gate = torch.sigmoid(
            self.fusion_gate(
                rearrange(
                    torch.cat([temporal_features, spatial_features], dim=2),
                    "b t c h w -> (b t) c h w",
                )
            )
        )
        gate = rearrange(gate, "(b t) c h w -> b t c h w", b=b, t=t)
        return x + gate * temporal_features + (1.0 - gate) * spatial_features


class SequenceSpatialTemporalMambaThenDownAttenfusionLayer(nn.Module):
    """Event/image encoder front-end followed by factorized spatio-temporal Mamba."""

    def __init__(self, in_channels, out_channels, num_block=1, norm=None,
                 use_atten_fuse=False):
        super(SequenceSpatialTemporalMambaThenDownAttenfusionLayer, self).__init__()
...
class AbSpatialTemporalMambaBlock(FinalBidirectionAttenfusion):
    """REFID ablation using factorized spatial-temporal Mamba in event encoders."""

    def __init__(self, *args, **kwargs):
        num_block = kwargs.get("num_block", 3)
        super(AbSpatialTemporalMambaBlock, self).__init__(*args, **kwargs)
